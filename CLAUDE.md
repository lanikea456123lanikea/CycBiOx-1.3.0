...
- **ROI处理**: 智能ROI区域选择，支持全图和区域处理模式，读取ROI数据
...
- **分割模型支持**: StarDist、Cellpose、InstanSeg、QuPath Detection四种分割模型，自动过滤和排序对应格式的测量值
  ...
  - Cellpose: "Nucleus: CD68 mean" 格式（空格分隔，小写统计量）
...
- **结果逻辑**: 一个细胞只能有一个标识符，如CD3+，那就是 CD3+NK1.1-CD8-……，是 unclassified，全阴性


### 3. 细胞分类管理（生物学表型引擎）
- **表型定义**: 用户自定义表型名称和marker states规则（阳性+/阴性-/无关+or-），支持复杂生物学分类逻辑
...
- **结果逻辑**:表型定义时候，阳性则赋予+，阴性则不显示这个marker，无关则累加两种情况，如只有两个marker，CD3阳性，CD31无关=CD3+ cd31+ 和 CD3+ 两种情况，全阴性匹配classification的unclassified
